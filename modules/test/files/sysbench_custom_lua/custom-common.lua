...
function set_vars()
   ...
   oltp_reconnect = oltp_reconnect or false
   oltp_rand_delay_min = oltp_rand_delay_min or 0
   oltp_rand_delay_max = oltp_rand_delay_max or 0

   if (oltp_rand_delay_max ~= 0 or oltp_rand_delay_min ~=0 ) then
      oltp_rand_delay = true
   else
      oltp_rand_delay = false
   end

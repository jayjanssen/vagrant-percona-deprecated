...
class percona::server-password {
    if $percona_server_version == "57" or $percona_server_version == "-57" {
        exec {"remove57randompassword":
            command => 'mysql -u root -p`cat /var/lib/mysql/error.log | grep "A temporary password is generated for root@localhost" | tail -n 1 | awk "{print \\$(NF)}"` --connect-expired-password -e "set password=\"\""',
